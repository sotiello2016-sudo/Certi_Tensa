...
const App: React.FC = () => {
  ...
  // Auto-fullscreen logic: trigger on first user interaction
  useEffect(() => {
    const handleFirstInteraction = () => {
      if (!document.fullscreenElement) {
        document.documentElement.requestFullscreen().catch(() => {
          // Ignore failures, browser might block if not enough "transient activation"
        });
      }
             ...
             <div className="flex items-center gap-2">
                 ...
                 <div className={`flex items-center rounded border ml-4 transition-colors ${!isAccessAllowed ? "opacity-50 pointer-events-none" : ""} ${state.masterItems.length > 0 ? "bg-emerald-100 border-emerald-300 text-emerald-800 shadow-sm" : "bg-white border-slate-300 text-slate-700 hover:bg-slate-50"}`}>
                     ...
                     {state.masterItems.length > 0 && (
                        <button className="px-2 py-2 border-l border-emerald-200 hover:bg-emerald-200 text-emerald-700 rounded-r focus:outline-none relative group cursor-help" onClick={(e) => { e.preventDefault(); }} aria-label="Información"><Info className="w-4 h-4" />
                 ...
                 <button onClick={toggleFullscreen} className={`flex items-center gap-2 px-3 py-2 bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 rounded ml-2 transition-colors ${!isAccessAllowed ? "opacity-50 pointer-events-none" : ""}`} title={isFullscreen ? "Salir de pantalla completa" : "Pantalla completa"} disabled={!isAccessAllowed}>{isFullscreen ? <Minimize className="w-4 h-4 text-slate-500" /> : <Maximize className="w-4 h-4 text-slate-500" />}</button>
